      ...
      try {
        ...
        await Promise.all(
          wordpressTags.map(async (tag) => {
            // replace wordpressTags with strapiTags
            if (tag) {
              if (tag) {
                try {
                  const tagFields = mapFieldsNest(tag, authorStructure?.tags); // comment this line
                  const tagExist = await strapi
                    .query("api::tag.tag")
                    .findOne({ where: { id: tag?.id } });
                  if (!tagExist) {
                    await strapi
                      .service("api::tag.tag")
                      .create({ data: tagFields }); //replace tagFields with strapiTags
                  } else {
                    console.log(`Tag with ${tag?.id} id already exists`);
                  }
                } catch (error) {
                  console.error(
                    `Error creating tag with ID ${tag.id}: `,
                    error
                  );
                }
              }
